...
init python:
    ...
    # def share_link_android(url):
    #     android.share_text(url)  # 링크 공유

    # def share_text_android(text):
    #     android.share_text(text)  # 텍스트 공유    
...
label splashscreen:
    ...
    with Pause(1)
    show text "{cps=3}{size=+60}{b}더{color=#000000} 運勢{/color}(운세){/b}{/size}{/cps}" with moveinbottom
    with Pause(1)
    hide text with dissolve
    with Pause(1)
    show text " {cps=2}{size=+40}{color=#000000}주역 지혜로 보는 {b}운세 (運勢) 풀이{/b}.{/color}{/size}{/cps} " with moveinbottom
    with Pause(3)
    hide text with dissolve
...
screen my_start_button():
    # 배경 이미지 추가
    add "images/android-presplash.jpg"  # 배경 이미지 파일
...
label start:
    ...
    #             show expression Text("풀이를 살펴봅시다!", style="body_text_style") at truecenter
    #             with Pause(2.0)
    #             hide text with dissolve
...
label juyuk_description:
    nvl clear
    with Pause(1)
    # show text "우주의 원리를 설명하는 주역 64괘로" with irisout
    show expression Text("우주의 원리를 설명하는 주역 64괘로", style="body_text_style") at truecenter
    with Pause(3)
    hide expression Text("우주의 원리를 설명하는 주역 64괘로", style="body_text_style") at truecenter
    # hide text with dissolve
    # show text "혼란스런 상황이 어떻게 흘러 갈지 조금만 앞서 볼까요?" with irisout
    show expression Text("혼란스런 상황이 어떻게 흘러 갈지 조금만 앞서 볼까요?", style="body_text_style") at truecenter
    with Pause(3)
    hide expression Text("혼란스런 상황이 어떻게 흘러 갈지 조금만 앞서 볼까요?", style="body_text_style") at truecenter
    # hide text with dissolve    
    # show text "{b}올바른 운세를 구하려면{/b}, 호흡과 주위를 정리하고," with irisout
    show expression Text("{b}올바른 운세를 구하려면{/b}, 호흡과 주위를 정리하고,", style="body_text_style") at truecenter
    with Pause(3)
    hide expression Text("{b}올바른 운세를 구하려면{/b}, 호흡과 주위를 정리하고,", style="body_text_style") at truecenter
    # hide text with dissolve
    # show text "{b}마음을 한곳으로 모은후에{/b} 진행해 주세요!" with irisout
    show expression Text("{b}마음을 한곳으로 모은후에{/b} 진행해 주세요!", style="body_text_style") at truecenter
    with Pause(3)
    hide expression Text("{b}마음을 한곳으로 모은후에{/b} 진행해 주세요!", style="body_text_style") at truecenter
    # hide text with dissolve
    return
...
label share_app:
    ...
    menu:
        "오아키":
            $ open_link("http://www.oarchi.co.kr")
        "Naver":
            $ open_link("https://www.naver.com")
        "Youtube":
...
label logic_tester:
    ...
    # call display_bricks_message(act_Narator,file_contents, 8)
...
label assemble_ramdom_bricks:
    ...
    show text "{size=60}{color=#330994}먼저, 괘를 구합니다.!{/color}{/size}" with irisout
    with Pause(0.8)
    hide text
    ...
    show text "{cps=8}[randomChoice]{/cps}" with  irisout
    # $ renpy.pause()
    with Pause(1.0)
    hide text    
    ...
    show text "{size=60}{color=#330994}괘를 찾았습니다.{/color}{/size}" with irisout
    # show expression Text("화면을 클릭해 주세요!", style="body_text_style") at truecenter
    $ renpy.pause()     
    # hide expression Text("화면을 클릭해 주세요!", style="body_text_style") at truecenter
    hide text with dissolve   
...
transform slow_fade_out:
    alpha 1.0  # 초기 투명도
    linear 30.0 alpha 0.0  # 3초 동안 투명도를 0으로 변경
...
label g_1111110:
    ...
    # $ renpy.pause()
    # hide text dissolve
    call display_bricks_message_v3(gMessage)
...
label g_222222:
    $ g_exp_messages = """
        곤위지 (坤爲地),
        위의 괘는 땅(地)이고 아래 괘도 땅(地)이다.
        모두 땅을 상징하는 곤괘(坤卦)로 구성되어 있어 지(地)라고 하였다.
        여섯 개의 효(爻)가 모두 음(陰,으로 하늘의 기를 받아들여 만물을 포용하고 양육시키는 어머니와 같다.
        곤(坤)이 없으면 건(乾)의 위대한 힘이 발휘되지 못한다. 곤(坤)의 속성은 순응하다.
        지극하다라는 뜻이다. 이 괘를 얻은 사람은 포용력과 인내를 가지고 유순한 마음으로 맡은 일에 충실해야 한다.
        또 봉사하는 자세로 사람들을 포용하고 사랑을 베풀어야 한다.
        자신의 능력을 과시하여 명성을 얻으려 하면 불상사가 생긴다.
        남을 돕더라도 남의 모르게 돕고, 영광이 돌아오더라도 남에게 돌려야 한다.
        말과 행동을 조심해야 한다.
        현재의 운세는 비교적 침체된 상태지만 머지않아 성운(成運)을 맞을 수 있다.
        윗사람이나 선배의 뜻에 잘 따라라.
        자신의 공적을 과시하려는 생각을 가지지 말고 남을 도와 그늘에서 묵묵히 일하라.
        부하가 상사를 앞지르려고 한다거나 아내가 남편을 앞서려고 하면 반드시 불상사가 일어난다.
        사업 확장이나 신규 사업에 착수하는 것은 좋지 않으므로 당분간 내실을 꾀하면서 관망하는 것이 좋다.
        """
...
label g_212221:
    ...
    $ g_exp_messages = """
        ...
        하늘과 땅이 자리를 정한 뒤에 천둥과 번개 그리고 폭우가 쏟아져 내리는 모습이라 할 수 있으니,
        지금은 모든 일에 어려움이 많다.
        마치 출산의 고통과도 같다. 그러나 출산의 고통 끝에 자식을 얻는 기쁨이 있는 것처럼 곧 좋은 때가 온다.
        날이 갈수록 운이 호전되기 때문에 초조하지 말고 인내력을 가지고 고통을 이겨내야 한다.
        지금은 매우 어려운 상황에 처해있기 때문에,,
        큰 목적이나 희망을 가지고 있더라도 주위의 상황이 불리하여 뜻을 이룰 수 없는 때다.
        무리하게 앞으로 나아가려는 것은 오히려 화를 초래한다.
        고진감래(苦盡甘來)의 진리를 명심하고 당분간 은인자중하며 때가 오기를 기다리는 것이 좋다.
        사업도 매사에 막히는 일이 많고 다사다난(多事多難)하므로,,
        매우 좋은 계획을 가지고 있더라도 당분간 관망하는 것이 좋다.
        비가 온 뒤에 땅이 굳어진다는 속담처럼 불행 뒤에 항상 행복이 따라온다는 자연의 진리를 명심해야 한다.
        시간이 지나면 자신의 포부를 펼 수 있는 기회가 온다.
        """
...
label g_122212:
    ...
    $ g_exp_messages = """
        ...
        그래서 교육을 뜻하는 몽(蒙)자를 괘의 이름으로 하였고 계몽(啓蒙)이라는 말이 여기서 나왔다.
...
label g_111212:
    ...
    $ g_exp_messages = """
        ...
        끝까지 자기의 뜻을 관철하려하지 말고 남의 의사를 존중하고 따르는 것이 좋다.
        싸워서 이겨도 득이 되지 않는다. 운세는 전반적으로 쇠운(衰運)이다.
        시비, 소송 등이 생기기 쉬우니 타인과 의견 충돌을 피해야 한다.
        자신을 억제하고 시간이 지나기를 기다리는 것이 최선이다.
        사업은 실력에 맞는 일이라면 아무 탈이 없겠지만 자기 분수에 맞지 않는 일에 착수하면 실패하기 쉽다.
        문서상의 일로 곤경에 빠질 우려가 있으니 조심해야 한다.
        """
...
label g_222212:
    ...
    $ g_exp_messages = """
        ...
        정확한 판단력과 과감한 실천력이 필요하며 매사에 있어서 주도면밀한 계획과 준비가 필요하다.
        또 일단 일을 시작하면 끝까지 밀고 나가야 한다.
        장수는 남보다 어려움이 많으므로 개인적인 감정이나 고집을 버리고 만사에 공명정대해야 한다.
...
label g_212222:
    $ g_exp_messages = """
        수지비 (水地比),
        위는 물(水)이고 아래는 땅(地)이다. 비(比)는 ‘견주다’ ‘비교하다’ ‘인화(人和)’를 상징한다.
        ...
        이 괘를 얻은 사람은 여러 사람들로부터 신망을 얻고 있어 집단의 우두머리로 손색이 없으나 경쟁이 치열하다.
        운세는 점진적으로 발전한다.
...
label g_112111:
    $ g_exp_messages = """
        ...
        비가 오면 만유의 생명체는 그 비를 저장하는 까닭에 저축한다는 의미로 축(畜)을 괘의 이름으로 하였다.
        이 괘를 얻은 사람은 하늘에서 바람이 불어 구름을 쫓는 상이어서 비는 안 오고 구름만 이리저리 몰려다니는 것처럼,,
        일이 될 듯 하면서도 안 되는 일이 많고,,
        대인 관계에서도 불화가 많다. 그렇기 때문에 안정된 생활을 영위하기 위해서는 미래를 준비하는 저축을 하여야 한다.
        운세는 경제적으로 안정된 시점에 와 있기는 하나 비가 오기전의 음울한 하늘과 같이 잡다한 어려움이 많다.
        운이 막혀있기 때문에 내실(저축)을 기하고 현상 유지에 힘써야 한다.
        새로운 사업에 손을 대면 고전하게 된다.
        """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_111211:
    $ g_exp_messages = """
        천택이 (天澤履),
        위는 하늘(天)이고 아래는 못(澤)이다. 이(履)는 ‘밟는다’ ‘따른다’ ‘예절’이라는 뜻이다.
...
label g_222111:
    $ g_exp_messages = """
        지천태 (地天泰),
        위는 땅(地)이고 아래는 하늘(天)이다. 태(泰)는 ‘크다’ ‘크게 통한다’ ‘태평하다’라는 뜻이다.
        땅의 기운은 하늘로 올라가고 하늘의 기운은 땅으로 내려와 서로 조화를 이루니,,
        만물이 생성되는 화창한 봄날이다.
        그러므로  태평하다는 태(泰)를 괘의 이름으로 하였다.
        이 괘를 얻는 사람은 만사가 형통하는 시점에 놓여있기 때문에 지금의 현실에 만족하는 것이 최선이다.
...
label g_111222:
    $ g_exp_messages = """
        ...
        하늘은 하늘대로 위에 있고 땅은 땅대로 아래에 있으니 천지 화합이 일어나지 않아 막혀 있는 상태이므로,,
        답답하다는 뜻으로 비(否)를 괘의 이름으로 하였다.
        이 괘를 얻은 사람은 모든 일이 침체되어 난처한 지경에 처해있다.
        하늘은 하늘  땅은 땅이라는 식으로 화합의 기미가 전혀 보이지 않고 반목의 상태가 지속된다.
        답답하고 안타깝겠지만 적당한 시기가 올 때까지 은인자중 하는 것이 상책이다.
        믿었던 사람들로부터 배반을 당하고 의사 소통이 되지 않아 마치 달이 먹구름 속에 숨은 상태다.
        사업 확장을 꾀하거나 신규 사업에 손을 댈 수 있는 상황이 아니므로 상황이 혼전될 때를 기다려야 한다.
        겨울이 오면 또 봄이 가까워진다는 사실을 명심할 일이다.
        """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_111121:
    $ g_exp_messages = """
        ...
        이 괘를 얻은 사람은 여러 사람과 협력하여 일을 도모하면 주변의 도움을 받으며 큰 일을 성취할 수 있다.
        그러나 동인(同人)이라는 뜻에는 같은 목적을 가진 사람이기 때문에 경쟁자라는 의미도 포함되어 있다.
        자신의 실속보다는 의리를 중요시하고 사적인 이익보다는 공적인 이익을 중요시하는 마음 자세가 필요하다.
        사업은 내부의 분쟁만 없다면 얼마든지 발전할 수 있으며,,
        운이 좋기 때문에 불의(不義)한 일만 아니라면 중도에 포기하지 말고 꾸준히 노력해야 한다.
        다만 경쟁자가 많기 때문에 약간의 고생은 예상해야 한다.
        """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_121111:
    $ g_exp_messages = """
        ...
        옛사람들은 금(金)과 옥(玉)이 집안 가득한 것을 여기서 인용하여 대유(大有)라고 표현하였다.
        그러나 차면 이지러지는 것이 또한 역(易)의 원리다.
        언제 닥칠지 모르는 쇠운(衰運)에 대비해야 하며 특히 남으로부터 시기와 원망을 사지 않도록 주의해야 한다.
        사업은 무리한 일만 아니라면 추진해도 별 어려움 없이 성취할 수 있다.
        그러나 한 낮이 가면 어둠이 점점 다가온다는 사실을 명심해야 한다.
        """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_221222:
    $ g_exp_messages = """
        뇌지예 (雷地豫),
        위는 우뢰 천둥(雷)이고 아래는 땅(地)이다. 예(豫)는 ‘예측한다’라는 뜻이다.
        땅 위 에서 천둥 번개가치면 비가 내리는 것을 예측할 수 있으므로 예(豫)를 괘의 이름으로 하였다.
        비가 내리면 대지를 골구려 적셔주어 땅속에 있던 씨앗이 싹을 터 이제 땅위로 올라올 수 있다.
        이 괘를 얻은 사람은 선견지명(先見之明)이 있는 해박한 사람으로 지금까지 노고가 인정되어 서서히 그 대가를 받을 수 있다.
        전부터 계획하고 있던 사업이나 자신의 포부를 펼 때다.
        예(豫)란 좋은 일에 대한 예측을 뜻하기도 하지만 나쁜 일을 미연에 방지하라는 뜻도 되기 때문에,,
        유비무환(有備無患,의 준비를 철저히 해야한다.
        하나의 양에 다섯의 음에 둘러싸여 있기 때문에 여난(女難)을 당할 수 있으므로 이성관계에 특별히 조심해야 한다.
...
label g_211221:
    $ g_exp_messages = """
        ...
        이 괘를 얻은 사람은 실력은 충분히 갖추고 있지만 약간 어려운 상황에 처해 있어 잠시 타의에 의해 움직이는 운세다.
        ...
        남이 자기를 따르게 하려면 자신이 먼저 남을 위해 솔선수범 해야 한다는 교훈을 명심해야 한다.
        """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_122112:
    $ g_exp_messages = """
        ...
        이 괘를 얻은 사람은 뽕잎이 누에에 야금야금 먹혀 들어가듯 심각한 상황에 놓여 심각한 고통을 당하고 있다.
        이것들은 모두 자업자득(自業自得)으로 문제의 원인이 자신에게 있다.
        즉 지금 겪는 고난은 과거에 저지른 자신의 그릇된 일에서 비롯된 것이므로 환골탈퇴(換骨脫退)하려는 굳은 의지가 필요하다.
        지금의 고난을 전화위복(轉禍爲福)의 기회로 삼아 평소 잘 아는 사람을 조심하고 자신의 좋지 못한 생활 태도를 청산해야 한다.
...
label g_222211:
    $ g_exp_messages = """
        지택임 (地澤臨),
        위는 땅(地)이고 아래는 목(澤)이다. 임(臨)은 ‘순서를 밟다’ ‘군림하다’라는 뜻이다.
        땅속에 있는 물이 지상으로 뚫고 올라오는 형상이므로 새로운 시작에 임한다는 뜻에서 임(臨)을 괘의 이름으로 하였다.
        그런데 임(臨)은 군림(君臨)한다는 의미가 있으므로 여러 사람 위에 있는 지도자의 상이다.
        이 괘를 얻은 사람은 정당한 방법으로 자신의 능력을 발휘하여 새로운 계획이나 포부를 실천에 옮기면 반드시 성공하여 우두머리가 된다.
        정치인이나 대표에게는 가장 좋은 괘다.
        여성이 이 괘를 얻으면 땅 밑에서 솟아오르려는 물처럼 앞 뒤 가리지 않고 유혹과 색정에 빠져들 수 있으니 조심해야 한다.
        사업은 왕성한 운세이므로 주저하지 말고 추진하는 것이 좋다.
        그러나 뜻하지 않은 변화가 속출하기 때문에 철저한 준비와 주변 사람들과 긴밀한 유대관계가 중요하다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_112222:
    $ g_exp_messages = """
        ...
        물질적인 것보다는 정신적인 면에서 더 큰 발전이 있으므로 학문, 연구, 종교와 관련되는 일에는 매우 좋다.
        ...
        정신수양이 안되면 변화에 적응하지 못해 심적으로 고통받게 된다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_121221:
    $ g_exp_messages = """
        화뇌서합 (火雷噬嗑),
        위는 불(火)이고 아래는 천둥 우레(雷)다. 서합(噬嗑)에서 서(噬)는 씹다라는 뜻이고,,
        ...
        이 괘를 얻은 사람은 격렬한 언쟁과 싸움에 휘말려 있고 서로 고집이 팽팽히 맞서 어느 쪽도 지지 않으려고 한다.
        어려움에 봉착해 있다.
        그러나 음식물을 다 씹어 삼키면 몸에 좋은 영양분이 되듯이 싸움이 끝나면 전보다 더욱 다정해 진다.
        한 바탕 옥신각신은 필연적이니 차라리 멋지게 싸우고 기분 좋게 화해하는 것이 현명한 방법이다.
        힘에 겨운 일이나 주변 사람들과 불화가 있지만 용기를 잃지 않고 노력한다면 머지않아 기쁜 일이 있다.
        사업은 처음에는 어려움이 많겠지만,,
        나중에는 크게 성공할 수 있으므로 새로운 사업을 벌이거나 사업 규모를 확장해도 좋다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_122121:
    $ g_exp_messages = """
        ...
        내적인 아름다움을 꾸민다는 뜻과 겉만 그럴 듯 하게 교언형색(巧言形色)으로 꾸민다는 뜻도 된다.
        내면의 세계에 충실한 사람은 매우 좋으나 그렇지 않은 사람에게는 허세를 부리거나 겉치레만 하다 실속이 없다.
        이 괘를 얻은 사람은 공연히 우쭐해지지 말고 분수에 맞는 생활을 하여야 한다.
        특히 남의 감언이설(甘言利說)이나 속임수 그리고 사기 등 금전상의 손실도 주의해야 한다.
        예술, 예능, 방송 등 화려한 일에 종사하는 사람은 인기를 얻고 자신의 역량을 발휘할 수 있다.
        사업은 내실(內實)있는 경영을 하여야 한다. 괜히 허세를 부리다가는 실패한다.
        광고, 실내장식, 미용, 의상, 양화 등의 일에는 매우 좋은 괘다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_122222:
    $ g_exp_messages = """
        ...
        내부에서 분규가 일어나 어려움을 겪을 수 있으며 동업자나 신임하던 부하에게 배신당할 염려가 있다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_111221:
    $ g_exp_messages = """
        천뇌무망 (天雷无妄),
        위는 하늘(天)이고 아래는 천둥 우레(雷)다.
        무(无)는 ‘없다’라는 뜻이고 망(妄)은 ‘허망하다’는 뜻이므로,,
        욕망도 작위도 없는 자연 그대로의 순리를 나타내 무망(无妄)을 괘 이름으로 하였다.
        하늘에서 천둥 우레가 울리니 머지 않아 비가 올 것은 분명하지만 지금은 비가 올 듯 올 듯 하면서 오지 않고 있다.
        제일 아래에서 외롭게 움직이는 양의 기운을 위의 하늘이 돕고 있으니 새로운 변화는 거스를 수 없는 대세다.
...
label g_122111:
    $ g_exp_messages = """
        ...
        이 괘를 얻은 사람은 어떠한 일에도 대길하다. 본인의 역량도 뛰어날 뿐 아니라 시운이 매우 좋다.
        노력하는 대로 그에 비례하여 결실을 얻을 수 있다.
        포부를 크게 갖는 것이 좋다. 사업은 무슨 일을 하든 비약적인 발전이 있으므로 적극적으로 나가는 것이 좋다.
        그러나 단숨에 일을 하려고 서두르면 안 된다.
        대축(大畜)에는 ‘머물다’ ‘기다리다’의 뜻도 있어 큰일을 이루기 위해서 준비한다는 의미도 되기 때문이다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_122221:
    $ g_exp_messages = """
        산뇌이 (山雷頤),
        위는 산(山)이고 아래는 천둥 우레(雷)다. 이(頤)는 ‘턱’ ‘기르다’ ‘봉양하다’의 뜻이다.
        ...
        처음에는 고난이 많고 이익이 적지만 나중에는 큰 이익을 얻을 수 있다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_211112:
    $ g_exp_messages = """
        ...
        잔잔한 못에 바람이 불어 물결이 크게 일어나니 조그만 배가 망망대해에서 풍랑을 만난 상과 같아,,
        지나치다라는 뜻에서 대과(大過)를 괘 이름으로 하였다.
        위아래 괘가 음효(陰爻)이고 가운데는 모두 양효(陽爻)이므로 바닥과 위는 약한데 가운데가 강해 지탱할 수 없음을 나타낸다.
        예를 들어 임금의 세력이 약하고 아래 백성 또한 지쳐있는데,,
        가운데 신하들이 권력을 강하게 마구 휘두르고 있으니 나라가 붕괴될 위험에 처해있는 상이다.
        이 괘를 얻은 사람은 모든 일에 크게 정도를 지나쳐 있다. 또 진퇴양난에 빠져있다.
        지극히 비정상적인 상황에 있으면서도 자신은 그것을 깨닫지 못하고 있으니 정신 차려 빨리 수습하지 않으면 큰 화를 입게 된다.
        대과(大過)란 ‘무거운 짐’이라는 뜻도 있으므로 잘못하면 과로에 쓰러지게 되므로 포기할 일이라면 미련 없이 포기하는 것이 좋다.
        사업은 겉보기에는 화려해도 실속이 전혀 없으니 확장을 꾀할 때가 아니다.
        내실을 기하여 현상유지에 힘써야 한다. 매우 불길한 괘이므로 대형사고 등에 조심해야 한다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_212212:
    $ g_exp_messages = """
        ...
        달관의 자세를 가지고 마음의 평정을 유지하는 것이 중요하다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_221112:
    $ g_exp_messages = """
        뇌풍항 (雷風恒),
        위는 천둥 우레(雷)이고 아래는 바람(風)이다. 항(恒)은 ‘변함이 없다’ ‘한결 같이 계속 된다’라는 뜻이다.
...
label g_111122:
    $ g_exp_messages = """
        ...
        산이 아무리 높다하더라도 하늘 아래 있고, 밑에 있는 두 음효(陰爻)가 위에 있는 양효(陽爻)를 밀어내는 형상이니,,
        이제 물러나라는 뜻에서 둔(遯)을 괘 이름으로 하였다.
        이 괘를 얻은 사람은 아무리 자신이 올바른 일을 해도 남들이 인정해주지 않고,,
        오히려 시기와 모함을 받으므로 한 걸음 물러서 때를 기다리는 것이 좋다.
        지금 역경에 처해 있더라도 곧 역경에서 벗어날 수 있다. 대체로 이 괘는 36계 줄행랑이 묘수다.
        사업은 전반적으로 침체가 되어 있으므로 서두르지 말고 착실하게 내실을 기하는 것이 좋다.
        안될 일이라면 용단을 내려 미련 없이 손을 떼는 것이 오히려 이익일 수 있다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_221111:
    $ g_exp_messages = """
        뇌천대장 (雷天大壯),
        위는 천둥 우레(雷)이고 아래는 하늘(天)이다. 대장(大壯)을 ‘힘차다’ ‘성대하다’ ‘씩씩하다’라는 뜻이다.
        하늘 위에서 우레가 움직이고 있고 아래에 있는 네 개의 왕성한 양이 위에 있는 두 개의 음을 몰아 붙이고 있으므로,,
        힘차고 씩씩하다는 뜻에서 대장(大壯)을 괘 이름으로 하였다.
        이 괘를 얻은 사람은 대단한 성운(盛運)으로 매사에 의욕이 있고 체력도 강하여 적극적으로 일을 추진한다.
...
label g_121222:
    $ g_exp_messages = """
        ...
        불인 태양이 지상 위로 점점 떠오르면서 밝아지니 나아간다는 의미에서 진(晋)을 괘 이름으로 하였다.
        태양이 높이 오를수록 어둠은 사라지고 천하가 밝아진다.
        이 괘를 얻은 사람은 운세가 서서히 강해지니 포부를 펼 때며 만사가 뜻대로 이루어진다.
        그러나 태양이 아무리 밝아도 구름에 가려져 그 위세를 잃을 수도 있으니 항상 성실하고 근면한 자세로 임해야 한다.
        주위에 질투, 시기, 모함 등이 많을 때이므로 넓은 아량과 용기로서 이러한 장애를 극복해야 한다.
        사업은 새로운 것에 착수할 좋은 기회다. 경쟁이 심해도 성공할 수 있다.
        그러나 서서히 좋아지므로 서둘면 실패한다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_222121:
    $ g_exp_messages = """
        ...
        태양이 땅 아래 지하에 잠겨가고 있으니 어두움이 온다는 뜻에서 명이(明夷)를 괘 이름으로 하였다.
        이는 태양이 서산에 지는 상이다.
        이 괘를 얻은 사람은 매우 불길한 운세에 놓여 있다.
        자신의 지식이나 역량을 과신하여 섣불리 행동하면 오히려 비방과 모함을 당하여 위험에 빠질 수 있다.
        밝은 낮의 선은 사라지고 어두운 밤의 악이 지배하는 시간이니 모든 일에 손을 떼고 충분한 휴식을 취하는 것이 좋다.
        밤이 지나면 다시 밝은 내일이 온다.
        그때까지 내면의 실력을 쌓으며 참고 기다려야 한다.
        사업은 당분간 관망하는 것이 좋다.
        현상 유지도 힘들 때다.
        새로운 계획이 있더라도 기다렸다가 다시 생각해보아야 한다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_112121:
    $ g_exp_messages = """
        ...
        또 불 위에 바람이 불고 있으니 불씨를 꺼뜨리지 않고 잘 간수하는 것이 옛 여자들의 중대사였고 집안을 다스리는 기본이었다.
        이 괘를 얻은 사람은 외부로 나서서 활동을 하는 것보다는 차분하게 내부 일을 정비하는 것이 좋다.
...
label g_121211:
    $ g_exp_messages = """
        화택규 (火澤睽),
        위는 불(火)이고 아래는 못(澤)이다. 규(睽)는 ‘서로 등지다’ ‘노려보다’ ‘사팔눈’이라는 뜻이다.
        불은 타오르면서 위로 올라가고 연못의 물은 낮은 쪽으로 흘러가니 서로 등져 떨어지므로 규(睽)를 괘 이름으로 하였다.
        이 괘를 얻은 사람은 매사에 장애가 많고 다툼과 근심 속에서 지낸다.
        다른 사람과 전혀 의사가 통하지 않고 자신이 아무리 올바르게 행동하여도 남들은 그것을 옮게 보아주지 않는다.
        누구에게도 도움을 받을 수 없어 소외감이 생겨 자포자기하기 쉽다.
        아무리 자신이 정당하더라도 싸우지 말고 화해하는 것이 좋으며 언행을 조심해야 한다.
        사업은 아무리 좋은 계획이더라도 지금 서둘러 추진하면 내부에서 분쟁이 생기거나 사고가 나 실패하기 십상이다.
        겸허한 마음으로 자중하면서 고난의 때가 무사히 넘어가기를 기다려야 한다.
        전 임직원이 화합하여 일치 단결하는 것이 중요하다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_212122:
    $ g_exp_messages = """
        ...
        험난한 일들을 많이 당한다. 이 때는 부질없이 시간을 낭비하지 말고 수양과 사색에 힘쓰는 것이 현명하다.
        하던 일을 중지하고 잘못된 것을 찾아내는 것이 화를 최소화하는 방법이다.
        사업은 정리할 것은 정리하고 사업규모를 축소할 때다.
        사업 확장이나 신규 사업 참여는 반드시 실패한다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_221212:
    $ g_exp_messages = """
        뇌수해 (雷水解),
        위는 천둥 우레(雷)이고 아래는 물(水)이다. 해(解)는 ‘해결되다’ ‘해소된다’ ‘풀린다’라는 뜻이다.
        천둥 우레가 진동하여 비를 내리니 얼어붙었던 대지가 풀리는 봄을 의미하므로 해(解)를 괘 이름으로 하였다.
        이 괘를 얻은 사람은 지금까지의 고난이 해소되므로 모든 일이 술술 잘 풀려나가는 운세다.
        계획했던 사업이나 마음먹었던 일이 있으면 실천에 옮길 수 있는 좋은 기회다.
        해(解)는 문제 해결의 근본 원칙을 제시하고 있으므로,,
        지난날 어려웠던 시절의 과실과 허물은 서로 덮어주고 용서하는 것만이 화합으로 나가는 지름길이다.
        그러나 해(解)는 ‘쪼갠다’ ‘분해한다’라는 뜻도 있으므로 지금까지의 상태가 무너져 파국에 이를 수도 있다.
        순조롭게 진행되어 왔던 일이라도 방심하거나 노력을 게을리 하면 나쁜 결과를 초래할 수 있다.
        사업은 열심히 노력하면 모든 일이 순조롭게 풀린다.
        새로운 단합이 필요하고 공동 사업을 꾀하면 순조롭게 진행될 것이다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_122211:
    $ g_exp_messages = """
        ...
        그러나 일반적으로 생각하는 잃거나 빼앗기는 손실과 손해가 아니라 주는 희생을 말한다.
        ...
        현재 적자를 보고있더라도 장래성만 있다면 계속해도 좋다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_112221:
    $ g_exp_messages = """
        ...
        지금까지 노력했던 일이 결실을 맺으니 적극적으로 일을 추진하면 큰 이익을 얻을 수 있다.
...
label g_211111:
    $ g_exp_messages = """
        택천쾌 (澤天夬),
        위는 못(澤)이고 아래는 하늘(天)이다. 쾌(夬)는 ‘물리친다’ ‘결단한다’는 뜻이다.
        아래 다섯 양효(陽爻)가 위에 있는 하나의 음효(陰爻)를 밀어내고 있는 상이니 쾌(夬)를 괘 이름으로 하였다.
        소인배인 음효 하나가 군자인 다섯 양효 위에 군림하고 있으니 전체를 위해 소인을 물리칠 결단을 내릴 때다.
        또 하늘 위에 큰물이 있으니 홍수가 날 것이며 홍수는 만물을 휩쓸고 나간다.
        이 괘를 얻은 사람은 결단을 내릴 때다. 우유부단한 마음은 중요한 일을 놓치기 쉽다.
...
label g_111112:
    $ g_exp_messages = """
        ...
        하늘 아래에서 바람이 부니 흩어졌던 구름이 모이므로 만난다는 뜻의 구(姤)을 괘 이름으로 하였다.
        제일 아래의 하나의 음이 다섯 개의 양을 떠받치고 있으니,,
        한 여자가 다섯 남자를 상대하는 창녀와 같다하여 추하다는 의미도 있다.
        이 괘를 얻은 사람은 좋은 일보다는 나쁜 일을 만날 가능성이 훨씬 크므로 모든 일에 장애가 많다.
        모든 일을 신뢰할만한 사람과 의논하여 처리하고 항상 자신의 위치와 분수를 지켜야 화를 면할 수 있다.
        특히 부정하고 기가 센 여자를 만나 뜻밖의 화를 당할 수 있으니 조심해야 한다.
        사업은 현재의 상태를 유지하는데 최선의 노력을 해야한다.
        함부로 다른 일을 하였다가는 크게 실패할 염려가 있다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_211222:
    $ g_exp_messages = """
        택지췌 (澤地萃),
        위는 못(澤)이고 아래는 땅(地)이다. 췌(萃)을 ‘모인다’라는 뜻이다.
...
label g_222112:
    $ g_exp_messages = """
        지풍승 (地風升),
        위는 땅(地)이고 아래는 바람(風)이다. 승(升)을 ‘위로 상승하다’ ‘올라가다’ ‘번성하다’라는 뜻이다.
        ...
        이 괘를 얻은 사람은 소질과 능력을 충분히 갖추고 있는 상태이므로 머지않아 자신을 꽃피울 수 있다.
        왕성한 운세로 무엇을 하든 성공할 수 있는 괘다. 그러나 급진적인 것은 좋지 않다.
        점차적으로 하나하나 전진해 나가야 하며 성급히 서두르면 실패한다.
        어떠한 일을 해도 성공할 수 있으므로 새로운 일을 시작해도 좋고 현재하고 있는 일을 해도 좋다.
        너무 서두르지 않고 충분하게 내실을 다지면서 전진해 가면 반드시 성공한다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_211212:
    $ g_exp_messages = """
        택수곤 (澤水困),
        위는 못(澤)이고 아래는 물(水)이다. 곤(困)을 ‘부족하다’ ‘곤궁하다’ ‘괴롭다’ ‘통하지 않는다’라는 뜻이다.
        연못 아래에 있는 물이 빠지는 모습이니 물이 부족한 대지의 만물이 곤궁에 처하게 되므로 곤(困)을 괘 이름으로 하였다.
        곤(困)이라는 글자도 나무가 울타리 안에 갇혀 뿌리를 뻗을 수 없으니,,
        밖으로 성장할 수 없고 자유롭게 행동할 수 없는 곤궁의 의미를 뜻하고 있다.
        이 괘를 얻은 사람은 사방이 곽 막힌 상태로 생계조차도 어려울 정도로 극심한 곤궁에 처해있다.
        아무리 노력해도 성과가 없으니 힘만 들뿐이다.
        이럴 때일수록 현실을 직시하고 인내하며 한 걸음 뒤로 물러서 다음 기회를 기다리는 것이 좋다.
        사업은 아무리 유망한 사업이라도 실천에 옮기면 실패하므로 은인자중(隱忍自重,하면서 현상유지에 힘써야 한다.
        때를 기다리면 반드시 혼전될 때가 온다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_212112:
    $ g_exp_messages = """
        수풍정 (水風井),
        위는 물(水)이고 아래는 바람(風)이다. 정(井)을 ‘우물’ ‘두레박’을 뜻한다.
        ...
        큰 욕심을 버려야 하고 남에게 베풀어야 한다. 물이 고여 있으면 섞기 때문이다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_211121:
    $ g_exp_messages = """
        택화혁 (澤火革),
        위는 못(澤)이고 아래는 불(火)이다. 혁(革)을 ‘바꾸다’ ‘혁신하다’ ‘혁명’의 뜻이다.
        연못아래 불이 있으니 물이 끊어 증발하여 큰 변화를 하므로 혁(革)을 괘 이름으로 하였다.
        혁(革)을 짐승 가죽이라는 뜻으로 가죽의 털을 벗기면 전혀 다른 것으로 변하기 때문에 혁명이라는 의미가 있다.
...
label g_121112:
    $ g_exp_messages = """
        화풍정 (火風鼎),
        위는 불(火)이고 아래는 바람(風)이다. 정(鼎)을 ‘발이 셋인 솥’ ‘안정감’을 뜻한다.
        ...
        공생(共生,, 공존(共存,, 공영(共榮)을 신조로 인화(人和)을 잘하고 이익 분배에서도 공평해야 한다.
        사업은 초창기 혼란이 안정되어 가는 시점에 있다.
        주변 사람들과 일치 협력하여 일을 하면 크게 성공한다. 그러나 화합이 안되면 크게 실패한다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_221221:
    $ g_exp_messages = """
        진위뇌 (震爲雷),
        위도 천둥 우레(雷)이고 아래도 천둥 우레(雷,다.
        뇌(雷)을 ‘천둥 우레’ ‘몹시 두려워하다’ ‘사나운 모양’ ‘위엄을 떨치다’를 뜻한다.
        우레가 크게 진동하니 많은 사람들이 놀라 두려워하고 있다는 뜻에서 소성괘 이름 그대로 뇌(雷)을 괘 이름으로 하였다.
        이 괘를 얻은 사람은 새로운 변화를 예고하고 있으므로 항상 대처할 준비를 하고 있어야 한다.
        진(震,괘는 장남을 뜻하므로 집안의 어른으로서 명성을 얻거나 우두머리로서 중대한 결정을 내려야 할 때가 되었다.
        왕성한 활동력을 가지고 있고 어떠한 난관도 돌파할 수 있는 용기와 능력이 있으므로 무슨 일에나 성공할 수 있다.
        그러나 모든 일에 신중히 대처해야하므로 말을 삼가고 빈틈없는 계획과 침착성을 가져야 한다.
        사업은 한가지 일에만 총력을 기울인다면 놀라운 성공을 할 것이다.
        두 용이 구슬을 놓고 다투는 상이므로 매사에 경쟁이 심하다.
        너무 서둘거나 기분에 사로잡히면 실속을 얻지 못하니 내실을 기하여야 한다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_122122:
    $ g_exp_messages = """
        ...
        간(艮)괘는 하나의 양이 두 음 위에 있으니 그 이상 올라갈 수가 없어 그곳에 머무르고 있는 상이고,,
        산은 움직이지 않고 그곳에 있으므로 ‘머무르다’라는 뜻이다.
        이 괘를 얻은 사람은 지금 난관에 봉착해 있다. 할 일은 태산같이 많은데 능력이 부족하므로,,
        앞으로 나가는 것을 중지하고 힘을 길러야 한다.
        아무리 곤란한 처지에 있더라도 배신이나 변절을 하지 말고 굳은 신념을 갖고 지조를 지키면 곧 좋은 때가 온다.
        사업은 여러 가지 일에 손을 대면 실패하니 한가지 일에 전심전력하여야 한다.
        산 넘어 산이니 분수를 모르고 경거망동하면 안되고 산처럼 묵묵하게 꾸준히 노력하면 곧 그 산을 넘게 된다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_112122:
    $ g_exp_messages = """
        ...
        이 괘를 얻은 사람은 서서히 나아지는 것을 암시하고 있으니 인내심을 가지고 성실 근면하면 점진적으로 발전한다.
        비약적인 발전이나 허영을 꿈꾸지 말고 늦더라도 차근차근하게 일을 하는 것이 바람직하다.
        대기만사성(大器萬事成)이므로 아무리 큰 이익이 눈앞에 있더라도 순서와 절차를 지켜 나가야 된다.
        사업은 서서히 발전해 나가므로 서둘지 말고 투기 사업이나 지나친 확장을 하지 마라.
        크게 무리만 하지 않는다면 조금씩 만족스러운 성공을 해나갈 것이다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_221211:
    $ g_exp_messages = """
        뇌택귀매 (雷澤歸妹),
        위는 천둥 우레(雷)이고 아래는 못(澤)이다. 귀매(歸妹)는 ‘정상적이지 못한 결혼’이라는 뜻이다.
        위는 나이든 남자를 상징하는 진(震)괘이고 아래는 어린 여자를 상징하는 손(巽, ☴,괘다.
        이는 젊은 여자가 중년 남자와 결혼하는 것으로 여자에게 음란한 소질이 있어 남자를 따라다니니,,
        정상적이지 못하다는 뜻에서 귀매(歸妹)를 괘 이름으로 하였다.
        이 괘를 얻은 사람은 한순간의 잘못에 의해 두고두고 후회할 수 있으므로 시작을 잘해야 한다.
        처음에는 그럴 듯해도 끝이 나쁠 수 있으므로 매사에 신중하고 각별히 조심해야 한다.
        배우자의 선택을 잘못하여 일생을 망칠 수 있으므로 일시적인 감성에 치우치지 말고 냉철한 이성이 필요하다.
        사업은 한 발짝 뒤로 물러서 수동적인 태도를 취하고 남보다 앞장서지 말아야 한다.
        현상유지를 하는 것이 최선이므로 새로운 일이나 확장은 금물이다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_221121:
    $ g_exp_messages = """
        뇌화풍 (雷火豊),
        위는 천둥 우레(雷)이고 아래는 불(火)이다. 풍(豊)은 ‘풍성하다’라는 뜻이다.
        ...
        그러나 풍성한 가을 뒤에는 추운 겨울이 오므로 점점 쇠운(衰運)이 도래하고 있음을 명심해야 한다.
        지금의 성운(盛運)을 유지하려면 겨울 준비를 하듯이 미래를 위한 준비를 철저히 해야 한다.
        사업은 지금이 절정이기 때문에 확장이나 신규 사업은 금물이고 오히려 조금씩 사업 규모를 줄여 내실을 기해야 한다.
        앞으로 다가올 혹독한 불황을 견디려면 불필요한 인원과 지출을 줄이고 저축을 해놓아야 한다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_121122:
    $ g_exp_messages = """
        ...
        여(旅)을 ‘여행’ ‘집과 고향을 떠나 낯선 곳으로 가는 것’ ‘방황하는 나그네’를 뜻한다.
        산 위에 태양이 있는데 산은 머물러 움직이지 않으므로 여관(旅館,과 같고, 불은 움직여 머무르지 않으므로,,
        여행자와 같으므로 여(旅)을 괘 이름으로 하였다.
        또 태양이 산에서 떠서 산으로 지는 것은 나그네의 여정과 같으므로 여(旅)을 괘 이름으로 한 것이다.
        이 괘를 얻은 사람은 길고 긴 여정에 지칠 대로 지친 고통스러움에 처해있다.
        지금의 여행은 즐거운 것이지만 옛날의 여행은 고행이므로 모든 것이 불안정하고 고독과 번뇌가 끊이지 않는다.
        여행할 때는 의지할 사람이 아무도 없기 때문에 매우 우울하고 절망적이지만,,
        대신 자신의 내면세계를 성장시킬 수 있는 좋은 기회다.
        참고 견디면 모든 것이 희망으로 돌아선다.
        대부분 새로운 환경에 처하게 되어 당황하는데 시간이 해결해주므로 때와 장소에 따라 잘 적응하도록 힘써야 한다.
...
label g_112112:
    $ g_exp_messages = """
        ...
        봄바람은 환절기이므로 뜻밖의 불상사를 당하고 남의 감언이설(甘言利說,에 잘 속는 결점을 가지고 있다.
        스스로 자신의 결점을 알고 확고한 주관을 가지고 행동하여야 성공할 수 있다.
        사업은 모든 것이 부드럽게 해결되는 듯 하다가도 큰 고비를 맞을 수 있다.
        신규 사업을 벌여 확장하기보다는 지금까지 쌓아온 기반을 토대로 일부분 확장하는 것이 좋다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_211211:
    $ g_exp_messages = """
        태위택 (兌爲澤),
        위도 못(澤)이고 아래도 못(澤)이다. 태(兌)을 ‘즐거움’ ‘온화한 분위기’를 뜻한다.
        위아래가 모두 연못이니 소성괘 이름 그대로 택(澤)을 괘 이름으로 하였다.
        태(兌, ☱,괘는 두 양이 아래에 있고 음이 위에 있어 부드러운 기쁨이 겉으로 나타나는 상이다.
        연못에 있는 물은 낮은 곳으로 흐르며 대지에 있는 모든 만물에게 골고루 물을 나누어주듯이,,
        베푸는 곳에서 기쁨을 느낄 수 있다.
        이 괘를 얻은 사람은 입이 두 개 겹쳐있으니 남달리 뛰어난 화술(話術)을 구사한다.
...
label g_112212:
    $ g_exp_messages = """
        풍수환 (風水渙),
        위는 바람(風)이고 아래는 물(水)이다. 환(渙)을 ‘흩어지다’ ‘풀어지다’라는 뜻이다.
...
label g_212211:
    $ g_exp_messages = """
        수택절 (水澤節),
        위는 물(水)이고 아래는 못(澤)이다. 절(節)을 ‘절도’ ‘규칙이나 제한’ ‘절약’을 뜻한다.
        ...
        사업은 내실을 기하며 현상유지 한다는 자세로 일을 하면 착실하게 이익을 올릴 수 있다.
        큰 사업에 무리하게 뛰어든다면 맵새가 황새를 쫓아가는 꼴이 된다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_112211:
    $ g_exp_messages = """
        풍택중부,
        위는 바람(風)이고 아래는 연못(澤)이다. 중부(中孚)는 ‘어미 새가 알을 품어 따뜻하게 한다’는 뜻이다.
        ...
        그러나 여러 사람의 도움에 우쭐하여 잘난 체하거나 나태해지면 당번에 나락(奈落,으로 떨어진다.
        사업은 비록 현재 부진한 상태에 있더라도 인내와 성의를 가지고 노력하면 머지 않아 성공한다.
        역량에 맞지 않는 일은 하지 말고 혼자보다는 주위의 도움을 받으면 더욱 좋다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_221122:
    $ g_exp_messages = """
        뇌산소과 (雷山小過),
        위는 천둥 우레(雷)이고 아래는 산(山)이다. 소과(小過)을 ‘조금 지나치다’라는 뜻이다.
        상괘와 하괘가 등을 지고 있고 음이 양에 비해 약간 많다는 의미에서,,
        조금 지나치다라는 뜻의 소과(小過)을 괘 이름으로 하였다.
        이 괘를 얻은 사람은 모든 일에 신중해야 하며 함부로 까불고 날뛰다가는 위험에 빠지게 된다.
        특히 대인 관계에 있어서 불상사가 많으므로 매사에 조심해야 한다.
        때에 따라서는 비굴하게 보일 만큼 저자세를 취하며 은인자중(隱忍自重,하는 것도 좋은 방법이다.
        사업은 신규 사업이나 확장을 꾀하는 것은 실패하여 패가망신할 수 있다.
        내부의 분규나 사고 등에 조심하고 규모를 줄여 내실을 기해야 한다.
        돌다리도 두들겨 보고 건너는 자세로 매사에 신중해야 한다.
    """
    call display_bricks_message_v3(g_exp_messages)
    return
...
label g_212121:
    $ g_exp_messages = """
        ...
        이 괘를 얻은 사람은 일생을 통하여 가장 왕성한 절정에 있는 운세로 더 이상 바랄 것이 없는 상태다.
        이제부터는 운세가 서서히 내리막길로 접어들기 때문에 현실에 안주하지 말고,,
        확실한 기반을 다져 쇠퇴를 막고 지금의 성운(盛運)을 계속 누릴 수 있도록 해야 한다.
...
label g_121212:
    $ g_exp_messages = """
        화수미제 (火水未濟),
        위는 불(火)이고 아래는 물(水)이다. 미제(未濟)란 ‘미완성’을 뜻한다.
        불과 물이 각기 제자리에 있기 때문에 아직 건너지 않았다는 뜻에서 미제(未濟)을 괘 이름으로 하였다.
        이 괘를 얻은 사람은 지금까지의 모든 일이 미완성인체 이루어지지 않았음을 뜻하고 있으니,,
        어그러지고 시의(時宜)에 맞지 않을 수 있다.
        현재는 매우 우울하고 절망적이지만 시간이 지나면 호전되므로 좌절하거나 포기하지 말아야 한다.
        겨울이 지나면 봄이 온다는 자연의 이치대로 그때를 대비하여 계획을 세우고 실력을 쌓아야 한다.
